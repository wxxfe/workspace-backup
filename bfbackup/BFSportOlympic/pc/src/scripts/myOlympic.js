...
require(['components/slider','components/player','components/navigation'], function (Slider,Player,Navigation){
    ...
    //前线巨星
    new Slider({wrap: $('.superstar .slide-wrap'), prev: $('.prev'), next: $('.next')});
    ////如果有多屏则显示箭头按钮
    if ($('.superstar .slide-wrap > ul > li').length > 1) {
        $('.superstar .data-wrap > a').show();
    }

    //名人榜
    new Slider({wrap: $('.slide-box .slide-wrap'), prev: $('.prev'), next: $('.next')});
    ////如果有多屏则显示箭头按钮
    if ($('.slide-box .slide-wrap > ul > li').length > 1) {
        $('.slide-box .data-wrap > a').show();
    }
