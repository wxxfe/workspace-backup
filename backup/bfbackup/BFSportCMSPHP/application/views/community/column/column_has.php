    ...
    <div class="wrapper">
        <div class="content-wrapper" style="margin-left: 0;">
            <section class="content-header">
              <h1>“<?=$cinfo['title']?>”板块下分类</h1>
              <ol class="breadcrumb">
                <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
                <li class="active">“<?=$cinfo['title']?>”板块下分类</li>
              </ol>
            </section>
                ...
                <div class="row">
                    <div class="col-md-12" id="user-list">
                        <div class="box">
                            <div class="box-body">
                                <div class="col-md-3">
                                    <form id="add-form" method="post" action="/community/columncm/columnhasadd/<?=$cinfo['id']?>">
                                        <div class="input-group">
                                            <input id="thread_id" class="form-control" placeholder="要添加的话题ID" type="text" name="thread_id" />
                                            <span class="input-group-btn"><button class="btn bg-green btn-flat" type="submit"><i class="fa fa-plus"></i> 添加</button></span>
                                        </div>
                                    </form>
                                </div>
                                <div class="col-md-9 text-right">
                                    <a class="btn btn-default btn-flat" role="button" href="<?=base_url('/community/columncm/columnList')?>"><i class="fa fa-reply"></i> 返回列表</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-12" id="pending-video-list">
                        <div class="box">
                            <div class="box-header with-border">
                                <h3 class="box-title">“<?=$cinfo['title']?>”板块下分类</h3>
                                <!-- 
                                ...
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td><?=$thread['count']?></td>
                                            <td>
                                                <a href="javascript:void(0)" class="item-text-edit-sort" data-type="text" data-pk="<?=$thread['tid']?>" data-name="sort" data-title="修改排序">
                                                <?=$thread['display_order']?>
                                                </a>
                                            </td>
                                            <td>
                                                <a class="btn btn-flat btn-danger btn-xs btn-remove" role="button" href="javascript:void(0);" data-cid="<?=$thread['id']?>"><i class="fa fa-remove"></i> 删除</a>
...
$('#batch-remove,.btn-remove').on('click',function(){
    ...
    var threadId = $(this).data('cid');
    var tr = $(this).parents('tr');
    var target = this;
    if(this.id == 'batch-remove') newsId = $('#batch-id').val();

    swal(alertConfig,function(){
        $.post('<?=base_url("/community/columncm/columnhasremove")?>',{id : columnId, cid : threadId},function(d){
            if(d == 'success'){
...
$('.item-text-edit-sort').editable({
    url: '<?php echo base_url("/community/columncm/columnHasSort")?>',
    title: '输入分类排序',
