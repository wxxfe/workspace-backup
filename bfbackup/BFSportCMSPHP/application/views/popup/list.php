                    ...
                    <div class="box">
                        <div class="box-body">
                            ...
                            <div class="col-md-9 text-right">
                                <a class="btn btn-success btn-flat" role="button" href="<?=base_url('/popup/add')?>"><i class="fa fa-plus"></i> 添加</a>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="box-body">
                            <table class="table table-hover">
                                ...
                                foreach($list as $key => $info): ?>
                                    <tr>
                                        <td><?=$info['id']?></td>
                                        <td><?php if ($info['image']) { ?><img src="<?=getImageUrl($info['image'])?>" style="width: 100px;" title="<?=$info['title'] ?>" /><?php }?></td>
                                        <td><?=$info['title']?></td>
                                        <td><?=$info['url']?></td>
                                        <td><?=$info['platf']?></td>
                                        <td><?=$info['version']?></td>
                                        <td><?=$info['channel']?></td>
                                        <td><?=$info['start_tm']?></td>
                                        <td><?=$info['finish_tm']?></td>
                                        <td style="width: 120px;">
                                            <!--  <a class="btn btn-flat btn-info btn-xs" role="button" href="#"><i class="fa fa-eye"></i> 预览</a>-->
                                            <a class="btn btn-flat btn-warning btn-xs" role="button" href="<?=base_url('/popup/add?id='.$info['id']).'&redirect='.current_url();?>"><i class="fa fa-edit"></i> 编辑</a>
                                            <a class="btn btn-flat btn-danger btn-xs btn-remove" role="button" href="javascript:void(0);" data-nid="<?=$info['id']?>"><i class="fa fa-remove"></i> 删除</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                                </tbody>
...
<script>
    ...
    $('#batch-remove,.btn-remove').on('click',function(){
        ...
        swal(alertConfig,function(){
            $.post('<?=base_url("/popup/remove")?>',{id : slideId},function(d){
                if(d == 'success'){
    ...
    $('.user-list').on('click', '.editable-submit', function(){
        ...
        $.post("<?=base_url('/popup/updatesort') ?>", { 'id':id, 'sort':input_val }, function(json){
            if (!json.status) {
