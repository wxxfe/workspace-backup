    ...
    <div class="wrapper">
        <div class="content-wrapper" style="margin-left: 0;">
            <section class="content-header">
              <h1>头条资讯列表<small>News List</small></h1>
              <ol class="breadcrumb">
                        ...
                        <div class="box">
                            <div class="box-body">
                                ...
                                <div class="col-md-9 text-right">
                                    <?php if($type == 'search'): ?>
                                    <a href="<?=base_url('/ChannelNews/index/'.$tagId.'/top')?>" class="btn btn-default btn-flat pull-right" role="button"><i class="fa fa fa-reply"></i> 结束搜索</a>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="box-header" style="padding: 0px 10px;">
                                <div class="nav-tabs-custom">
                                    <ul class="nav nav-tabs pull-right">
                                        <?php if($type != 'search'): ?>
                                        <li <?php if($type == 'thread'): ?>class="active"<?php endif; ?>><a href="<?=base_url('/ChannelNews/index/'.$tagId.'/thread')?>">话题</a></li>
                                        <li <?php if($type == 'activity'): ?>class="active"<?php endif; ?>><a href="<?=base_url('/ChannelNews/index/'.$tagId.'/activity')?>">活动</a></li>
                                        <li <?php if($type == 'special'): ?>class="active"<?php endif; ?>><a href="<?=base_url('/ChannelNews/index/'.$tagId.'/special')?>">专题</a></li>
                                        <li <?php if($type == 'gallery'): ?>class="active"<?php endif; ?>><a href="<?=base_url('/ChannelNews/index/'.$tagId.'/gallery')?>">图集</a></li>
                                        <li <?php if($type == 'program'): ?>class="active"<?php endif; ?>><a href="<?=base_url('/ChannelNews/index/'.$tagId.'/program')?>">节目</a></li>
                                        <li <?php if($type == 'collection'): ?>class="active"<?php endif; ?>><a href="<?=base_url('/ChannelNews/index/'.$tagId.'/collection')?>">合集</a></li>
                                        <li <?php if($type == 'video'): ?>class="active"<?php endif; ?>><a href="<?=base_url('/ChannelNews/index/'.$tagId.'/video')?>">视频</a></li>
                                        <li <?php if($type == 'news'): ?>class="active"<?php endif; ?>><a href="<?=base_url('/ChannelNews/index/'.$tagId.'/news')?>">新闻</a></li>
                                        <li <?php if($type == 'top'): ?>class="active"<?php endif; ?>><a href="<?=base_url('/ChannelNews/index/'.$tagId.'/top')?>">置顶</a></li>
                                        <?php endif; ?>
                                    ...
                                    <tbody>
                                        <?php
                                            function getEditUrl($type,$id){
                                                ...
                                                switch($type){
                                                    case 'news':
                                                        $editUrl = base_url('/news/edit/update/'.$id).'?redirect='.current_url();
                                                        break;
                                                    case 'video':
                                                        $editUrl = base_url('/video/detailEdit/'.$id).'?redirect='.current_url();
                                                        break;
                                                    case 'gallery':
                                                    ...
                                                    case 'activity':
                                                        ...
                                                        break;
                                                    case 'program':
                                                        $editUrl = base_url('/program/edit/'.$id).'?redirect='.current_url();
                                                        break;
                                                    case 'special':
                                                        $editUrl = base_url('/special/add?id='.$id);
                                                        break;
                                                    case 'collection':
                                                        $editUrl = base_url('/collection/edit/'.$id).'?redirect='.current_url();
                                                        break;
                                                    case 'thread':
                                                        $editUrl = base_url('/community/thread/edit/'.$id).'?redirect='.current_url();
                                                        break;
                                                }
                                                return $editUrl;
                                            }
                                        ...
                                        <?php if(!empty($news)): ?>
                                            ...
                                            <?php if($type == 'top'): ?>
                                                ...
                                                <tr>
                                                    ...
                                                    <td><img src="<?=isset($item['info']['image']) ? getImageUrl($item['info']['image']) : '';?>" style="width: 100px;" alt="" /></td>
                                                    <td><?=$this->config->item('news_type')[$item['type']]?></td>
                                                    <td><?=$item['info']['id']?></td>
                                                    <td><?=isset($item['info']['publish_tm']) ? $item['info']['publish_tm'] : ''?></td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        ...
                                                        <?php if($item['sort'] == 2147483647) {?>
                                                            <a class="btn btn-flat btn-primary btn-xs btn-cancel-forever-top" role="button" href="javascript:void(0);" data-tid="<?=$item['tid']?>"><i class="fa fa-share "></i> 取消永久置顶</a>
                                                        <?php } else {?>
                                                            <a class="btn btn-flat btn-primary btn-xs btn-set-forever-top" role="button" href="javascript:void(0);" data-tid="<?=$item['tid']?>"><i class="fa fa-arrow-circle-up"></i> 永久置顶</a>
                                                        <?php }?>
                                                    </td>
                                                </tr>
                                                <?php endforeach; ?>
                                            ...
                                            <?php else: ?>
                                                ...
                                                <?php
                                                    $o++;
                                                    $editUrl = getEditUrl($stype,$item['id']);
                                                ?>
                                                <tr>
                                                    <td><?=$item['title']?></td>
                                                    <td><img src="<?=isset($item['image']) ? getImageUrl($item['image']) : '';?>" style="width: 100px;" alt="" /></td>
                                                    <?php if($type == 'search'): ?>
                                                    ...
                                                    <?php endif; ?>
                                                    <td><?=$item['id']?></td>
                                                    <td><?=isset($item['publish_tm']) ? $item['publish_tm'] : ''?></td>
                                                    <td>
                                                        <?php if($type == 'video'): ?>
                                                    ...
                                                    <td>
                                                        ...
                                                        <?php if($type == 'search'): ?>
                                                            ...
                                                            <?php if(@$item['has_tag']): ?>
                                                            <a class="btn btn-flat btn-danger btn-xs btn-remove" role="button" href="javascript:void(0);" data-tid="<?=$item['tid']?>" data-nid="<?=$item['id']?>" data-type="<?=$stype?>"><i class="fa fa-remove"></i> 移除标签</a>
                                                            <?php else: ?>
                                                            <a class="btn btn-flat btn-danger btn-xs btn-add-tag" role="button" href="javascript:void(0);" data-tid="<?=$item['tid']?>" data-nid="<?=$item['id']?>" data-type="<?=$stype?>"><i class="fa fa-plus"></i> 添加标签</a>
                                                            <?php endif; ?>

                                                        <?php else: ?>

                                                        <a class="btn btn-flat btn-danger btn-xs btn-remove" role="button" href="javascript:void(0);" data-tid="<?=$item['tid']?>" data-nid="<?=$item['id']?>" data-type="<?=$type?>"><i class="fa fa-remove"></i> 移除标签</a>

                                                        <?php endif; ?>

                                                        <?php if(@$item['is_top']): ?>
                                                        <a class="btn btn-flat btn-primary btn-xs btn-cancel-top" role="button" href="javascript:void(0);" data-rid="<?=$item['id']?>" data-stype="<?=$stype?>" data-tid="<?=$item['tid']?>"><i class="fa fa-share"></i> 取消置顶</a>
                                                        <?php else: ?>
...
$.fn.editable.defaults.url = '<?=base_url("/ChannelNews/updateSort")?>';
...
$('.table').on('click','.btn-set-forever-top',function(){
    var tid = $(this).data('tid');
    var rid = $(this).data('rid');
    var stype = $(this).data('stype');
    var btn = $(this);
    ...
    swal({
        ...
        $.get('<?=base_url("/ChannelNews/setForeverTop")?>/' + tid + '/' + rid + '/' + currentTag,function(d){
            if(d == 'success'){
                swal({title : "永久置顶成功!",text : "已推荐到永久置顶列表中！",type : "success"},function(){

                    window.location.reload();
                });
            }else{
                swal("永久置顶失败!", "永久置顶失败，请重试！", "error");
            }
        });
    });
});
...
$('.table').on('click','.btn-cancel-forever-top',function(){
    var tid = $(this).data('tid');
    var btn = $(this);
    swal({
        title: "你确定要取消吗?",
        ...
        $.get('<?=base_url("/ChannelNews/cancelForeverTop")?>/' + tid,function(d){
            if(d == 'success'){
                swal({title : "取消成功!",text : "已从永久置顶列表取消！",type : "success"},function(){
                    if(currentType == 'top'){
                        window.location.reload();
                    }else{
                        btn.html('<i class="fa fa-arrow-circle-up"></i> 置顶').removeClass('btn-cancel-top').addClass('btn-set-top');
