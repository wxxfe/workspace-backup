define(function(){
	 ...
	 */
	function setday(inputObj, dateFormatStyle, beginDate, endDate, lang,type,callback) {
		if (null == inputObj) {
			return null;
		}
		new Calendar(inputObj, dateFormatStyle, beginDate, endDate, lang , type , callback).show();
	}
	 ...
	 * @param   lang      0(English)|1(Chinese)  Default Language is 0(English).
	 */
	function setmonth(inputObj, dateFormatStyle, beginDate, endDate, lang , type , callback) {
		if (null == inputObj) {
			return null;
		}
		new Calendar(inputObj, dateFormatStyle, beginDate, endDate, lang, type,callback).show();
	...
	Calendar.prototype.style = function () {
		...
		strStyle += ".calendar {font-size:12px;width:270px; margin:0;padding:0px;border:1px solid #e5e5e5;background-color:#ffffff;}";
		strStyle += ".calendar ul {list-style-type:none; margin:0; padding:0;vertical-align:middle;}";
		strStyle += ".calendar li {float:left;background-color:#e8e8e8;display:inline-block;}.calendar b{font-weight:bold;}";
		strStyle += ".calendar .day li,.calendar .date li{float:left;width:36px;height:36px;line-height:36px;margin:0 0 2px 2px;text-align:center;}";
		strStyle += ".calendar .day li{font-weight:bold; background:none;} .calendar .date li{background-color:#EDF3F9;}";
		strStyle += ".calendar .today{ background-color:#ff7f18; color:#ffffff}";
		strStyle += ".calendar .month li{float:left;width:24.8%;height:20px;line-height:20px;text-align:center;background-color:#EDF3F9;}";
		strStyle += ".calendar li a{width:100%; height:100%;display:block; text-decoration:none; font-family:Tahoma; font-size:14px; color:#333}";
		...
		strStyle += ".selectThisYear, .selectThisMonth{text-decoration:none; margin:0px; color:#000; font-weight:bold;font-size:18px; color:#ffffff;}";
		...
		strStyle += ".calendarTitle{background:#6b6b6b;text-align:center;height:40px;line-height:20px;clear:both;width:100%;}";
		strStyle += ".calendarTitle li{height:40px;font-size:18px; color:#ffffff; line-height:40px;background:none; display:inline-block }";
		strStyle += ".calendarTitle .mark{text-decoration:none;color:#ffffff;font-family:Tahoma;font-size:18px;font-weight:normal;}";
		strStyle += ".calendarTitle #PrevYear, .calendarTitle #PrevMonth{margin-left:15px;cursor:pointer}";
		strStyle += ".calendarTitle #NextYtvYear, .calendarTitle #NextMonth{margin-right:15px;cursor:pointer}";
		strStyle += ".calendarTitle .year{margin-left:38px;}";
		strStyle += ".calendarTitle .month{margin-right:38px;margin-left:15px;}";
		strStyle += ".today{ background-color:#ff7f18; color:#ffffff}";
		strStyle += ".calendar li a.selectedDate{background-color:#00c0ef; color:#ffffff}";
		strStyle += ".today a { color:#ff7f18; }";
		strStyle += ".calendarBottom {text-align:center;width:100%; overflow:hidden;clear:both;width:100%;border-top:1px solid #ddd;}";
		strStyle += ".calendarBottom li{float:left;font-weight:bold;text-align:center;}";
		strStyle += "#emptyCalendar{width:27%; display:none;}";
		strStyle += "#selectCurrent{width:46%;width:64px; height:30px; line-height:30px; display:inline-block; margin:10px auto 10px 30px;background:none;border:1px solid #e6e6e6;cursor:pointer}";
		strStyle += "#closeCalendar{width:64px; height:30px; line-height:30px; float:right; display:inline-block; margin:10px 30px 10px auto;background:none;border:1px solid #e6e6e6;cursor:pointer;}";
		strStyle += "</style>";
		return strStyle;
	};
	 ...
	 */
	var selectedDate = '';
	function Calendar(inputObj, dateFormatStyle, beginDate, endDate, lang, type,callback){
		this.selectDate = '';
	...
	Calendar.prototype.draw = function (callback) {
		...
		var head = '<!doctype html><html lang="en"><head><meta charset="UTF-8">' + this.style() + '</head><body style="padding:0px;margin:0px;">';
		...
		lis += "<li id='PrevYear' class='mark'  title='Previous Year'>&laquo;</li>";
		if ("d" == this.type) {
			lis += "<li id='PrevMonth' class='mark'  title='Previous Month'>&lsaquo;</li>";
			lis += "<li class='year'><span id='selectThisYear' class='selectThisYear'>" + this.date.getFullYear() + "</span>" + Calendar.language["title"][this.lang][0] + "</li>";
			if (0 == this.lang) {
				lis += "<li class='month'><span id='selectThisMonth' class='selectThisMonth'>" + Calendar.language["months"][this.lang][this.date.getMonth()] + "</span></li>";
			} else {
				lis += "<li class='month'><span id='selectThisMonth' class='selectThisMonth'>" + (this.date.getMonth() + 1) + "</span>" + Calendar.language["title"][this.lang][1] + "</li>";
			}
			lis += "<li id='NextMonth' class='mark'  title='Next Month'>&rsaquo;</li>";
			lis += "<li id='NextYear' class='mark'  title='Next Year'>&raquo;</li></ul></div>";
			...
			for (i = 1; i <= thisMonthEndDate; i++) { // Current Month's Date
				...
				var timesTampOneDay = 60*60*24*1000;
				if (currentDate < (Date.parse(this.beginDate.replace(/-/g, '/')) - timesTampOneDay) || currentDate > Date.parse(this.endDate.replace(/-/g, '/'))) {
					lis += "<li class='lastMonthDate'>" + i + "</li>";
					continue;
				}
				lis += "<li class='thisMonth' title='" + currentDate.format("yyyy-MM-dd") + "'><a href='javascript:void(0);' ";
				if (currentDate.format("yyyy-MM-dd") == new Date().format("yyyy-MM-dd")) {
			...
			for (var i = 1; i <= 12; i++) {
				...
				if ((this.year + "-" + (i > 9 ? i : "0" + i)) == new Date().format("yyyy-MM")) {
		...
		this.bingEvent(callback);
	};

	/**
	 * Bind Click Event into Calendar
	 */
	Calendar.prototype.bingEvent = function (callback) {
		var calendar = this;
		...
		this.getElementById("selectCurrent").onclick = function () {
			...
			calendar.valueSelected(calendar.date,callback);
			calendar.hide();
		};
		...
		for (var i = 0; i < elements.length; i++) {
			elements[i].onclick = function () {
				...
				this.getElementsByTagName("a").className = 'selectedDate';
				selectedDate = this.title;
				calendar.valueSelected(calendar.date,callback);
				calendar.hide();
			}
		}
	...
	Calendar.prototype.valueSelected = function (date,callback) {
		callback(date.format(this.dateFormatStyle));
	...
	Calendar.prototype.setAutoHeight = function () {
		...
		var height = this.getElementById("calendar").offsetHeight;
		var width = this.getElementById("calendar").offsetWidth;
	...
	Calendar.prototype.show = function () {
		...
		this.panel.style.left = xy.x - 112 + "px";//这里的数值是为了定位修改的,不需要请去掉
	...
	var calendarEle = '<div id="calendarPanel" style="position:absolute;visibility:hidden;z-index:9999;bottom: 0;"><iframe id="calendarIframe" scrolling="no" frameborder="0" width="100%" height="100%"></iframe></div>';
	$("body").append(calendarEle);

	return {setday:setday,setmonth:setmonth};
});
