VideoSrcGetter = {
	...
	//			isLive = false;
			...
			var playUrl = VideoSrcGetter.getPlayUrl(videoInfo, isLive);
			if(playUrl) {
			...
			success: function(data) {
				if(data.status === 1) {
					if(Utils.browserInfo.isMobileDevice) {
						...
						var playUrl = VideoSrcGetter.getPlayUrl(data.data, isLive);
						defer.resolve(playUrl);
					}
	...
	getPlayUrl: function(videoInfo, isLive) {
		var rsl;
		if(videoInfo.play_url) {
			rsl = videoInfo.play_url;
		} else if(videoInfo.play_code2){
			var host = isLive ? 'live.baofengcloud.com/' : 'vod.baofengcloud.com/';
			if(videoInfo.isvr) {
				rsl = 'http://'+ host + '/48853043/player/cloud.swf?' + videoInfo.play_code2 + '&auto=1&si=1';
			} else {
				rsl = 'http://'+ host + '/48842737/player/cloud.swf?' + videoInfo.play_code2 + '&auto=1&si=1';
			}
		}
		return rsl; 
	},
