                        ...
                        <div class="box">
                            <div class="box-body">
                                ...
                                <div class="col-md-9 text-right">
                                    <a class="btn btn-success btn-flat" role="button" href="<?=base_url('/slide/add')?>"><i class="fa fa-plus"></i> 添加焦点图</a>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="box-body">
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        <?php 
                                        foreach($slidedata as $key => $slide): $o++; ?>
                                        <tr>
                                            <td>
                                                <a href="javascript:void(0)" class="item-text-edit" data-type="text" data-nid="<?=$slide['id']?>" data-pk="<?=$slide['id']?>" data-name="priority" data-title="修改排序">
                                                    <?=$o?>
                                                </a>
                                            </td>
                                            ...
                                            <td>
                                                <?php if ('html' == $slide['type']) { echo $slide['data']; } else { echo $slide['ref_id']; }?>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                ...
                                                <a class="btn btn-flat btn-warning btn-xs" role="button" href="<?=base_url('/slide/add?id='.$slide['id'])?>"><i class="fa fa-edit"></i> 编辑</a>
...
$('#batch-remove,.btn-remove').on('click',function(){
    ...
    swal(alertConfig,function(){
        $.post('<?=base_url("/slide/remove")?>',{id : slideId},function(d){
            if(d == 'success'){
...
$('.user-list').on('click', '.editable-submit', function(){
	...
	$.post("</?=base_url('/slide/updatesort') ?>", { 'id':id, 'sort':input_val }, function(json){
	    if (!json.status) {
...
$.fn.editable.defaults.url = '<?=base_url('/slide/updatesort')?>';
$('.item-text-edit').editable({
