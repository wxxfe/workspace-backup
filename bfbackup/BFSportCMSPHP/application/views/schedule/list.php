    ...
    <div class="wrapper">
        <div class="content-wrapper" style="margin-left: 0;">
            <section class="content-header">
              <h1>赛程列表<small>Schedule </small></h1>
              <ol class="breadcrumb">
                            ...
                            <div class="box-body">
                                <div class="nav-tabs-custom">
                                    <ul class="nav nav-tabs">
                                        <li <?php if ($curr_event_id == 0):?>class="active"<?php endif;?>><a href="<?=site_url('schedule/main/index')."/0/{$curr_date}"?>">全部</a></li>
                                        <?php foreach ($events as $event):?>
                                        <li <?php if ($curr_event_id == $event['id']):?>class="active"<?php endif;?>><a href="<?=site_url('schedule/main/index')."/{$event['id']}/{$curr_date}"?>"><?=$event['name']?></a></li>
                                        <?php endforeach;?>
                                    </ul>
                                </div>
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="date-wrap">
                                            <a href="<?=site_url('schedule/main/index')."/{$curr_event_id}/{$date_slide['week_prev_date']}"?>" class="prev">&lt;</a>
                                            <div class="slide-wrap">
                                                <ul class="matchs">
                                                    <li>
                                                        <?php foreach($date_slide['week_data'] as $day_info):?>
                                                        <a href="<?=site_url('schedule/main/index')."/{$curr_event_id}/{$day_info['date']}"?>" <?php if($day_info['date'] == $curr_date):?>class="active"<?php endif;?>>
                                            ...
                                            <a href="<?=site_url('schedule/main/index')."/{$curr_event_id}/{$date_slide['week_next_date']}"?>" class="next">&gt;</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
