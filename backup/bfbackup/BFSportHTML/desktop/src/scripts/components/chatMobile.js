...
define(['components/bflogin','components/socket','components/localStorage','lib/base64.min'], function (bflogin,socket,localStorage,Base64){
    ...
    return {
        ...
        connect : function(){
            ...
            $.ajax({
                ...
                var jsonObj = jQuery.parseJSON(serverList);
                var servers = jsonObj.data.servers;

                for(var i=0; i<servers.length; i++){

                    var sUrl = 'ws://' + servers[i] + '/api/v1/subscribe?id=' + that.matchId;
        ...
        chatMessage : function(msg){
            ...
            if(this.team[msgBody.user.favor] == 'home'){
                ...
                if(msgBody.emoji > 0){
                    messageHtml += '            <img src="http://static.sports.baofeng.com/emoji/' + msgBody.emoji + '.png" alt="" />';
                }else{
        ...
        supportMessage : function(msg){
            ...
            this.objs.homeProgress.animate({width : homeProgressNum});
            this.objs.guestProgress.animate({width : guestProgressNum});
        ...
        _onSupportActive : function(event){
            ...
            if(teamId == home){
                showData.data.likes[teams[1]] = parseInt(homeNum) + 1;
                showData.data.likes[teams[0]] = parseInt(guestNum);
            }else{
                showData.data.likes[teams[0]] = parseInt(homeNum);
                showData.data.likes[teams[1]] = parseInt(guestNum) + 1;
            }
