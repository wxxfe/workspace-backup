                    ...
                    <div class="col-md-12" id="pending-video-list">
                        ...
                        <div class="box box-primary">
                            <div class="box-header with-border">
                                <h3 class="box-title"><?=$info['title']?> - 编辑节目</h3>
                            </div><!-- /.box-header -->
                            ...
                            <div class="box-body">
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>
                                                <a href="javascript:void(0)" class="item-text-edit-sort" data-type="text" data-pk="<?=$item['tid']?>" data-name="priority" data-title="修改排序">
                                                    <?=$key+1?>
                                                </a>
                                            </td>
                                            <td><?=$item['title']?></td>
                                            <td><?=$item['id']?></td>
                                            <td>
                                                <a href="javascript:void(0)" class="item-text-edit-issueno" data-type="text" data-nid="<?=$info['id']?>" data-pk="<?=$item['id']?>" data-name="issue_no" data-title="修改排序" data-placeholder="暂无期号">
                                                <?php if (!empty($item['issue_no'])):?>
                                                <?=$item['issue_no']?>
                                                <?php else:?>
                                                暂无期号
                                                <?php endif;?>
                            ...
                            <div class="box-body">
                                <div class="row">
                                    <div class="col-md-12 text-center">
                                        <a class="btn btn-default btn-flat" role="button" href="<?php echo site_url('/program/videoSearch/').$info['id'].'/video'?>"><i class="fa fa-edit"></i> 添加视频</a>
                                        &nbsp;&nbsp;&nbsp;&nbsp;
                                        <a class="btn btn-default btn-flat" role="button" href="<?=base_url('/program/getlist')?>"><i class="fa fa-reply"></i> 返回列表</a>
                                    </div>
                                </div>
                            </div>
                        </div>
...
$.fn.editable.defaults.url = '<?=base_url("/program/updateSort")?>';
$('.item-text-edit-sort').editable({
    params : function(param){
        param.program_id = <?=$info['id']?>;
        return param;
    },
...
$('.item-text-edit-issueno').editable({
    url: '<?php echo base_url("/program/updateContent")?>',
    params : function(param){
        param.program_id = <?=$info['id']?>;
        return param;
    },
...
$('.btn-remove').on('click',function(){
    ...
    swal(alertConfig,function(){
        $.get('<?=base_url("/program/removeVideo")?>/' + cid + '/' + vid,function(d){
            if(d == 'success'){
                swal({title : "删除成功!",text : "视频已从节目删除！",type : "success"},function(){
                    window.location.reload();
                });
            }else{
                swal("删除失败!", "视频从节目删除失败，请重试！", "error");
            }
        });
    });
});
