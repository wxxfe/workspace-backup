 ...
 * 接口前缀
 */
var apiTopicPrefix = 'http://api.board.sports.baofeng.com/api/v1/h5/board/thread/';
/**
 * 图文话题列表
 * 参数：
 * id: 话题Thread.id
 ...
 */
var apiTopicOne = apiTopicPrefix + 'post/id';
/**
 * 图文话题我要发言
 * 参数：
 * user：用户认证信息
 ...
 */
var apiSportsLogin = 'http://api.board.sports.baofeng.com/api/v1/h5/board/login';
...
var Utils = {
     ...
     * 转换成b64编码
     */
    b64btoa: function (str) {
        return window.btoa(encodeURI(str));
    },
    b64atob: function (str) {
...
var DataConfig = {
    ...
    nickname: Utils.docCookiesGetItem('bfuname'),
...
var StyleRulesDownloadAD = React.createElement(Style, {
...
var ContentADComp = React.createClass({
    displayName: 'ContentADComp',

    render: function () {
        return React.createElement(
            'div',
            { style: StylesAppAD.row1, onClick: this.downloadHandler },
            React.createElement(
                'h1',
                { style: StylesAppAD.font1 },
                '想看看大家还在热议什么？快来下载暴风体育'
            ),
            React.createElement(
                'span',
                { style: StylesAppAD.font2 },
                '>'
            )
        );
    },
...
var ReplyListComp = React.createClass({
    displayName: 'ReplyListComp',

    render: function () {
        var end = this.props.data.length - 1;
        var list = this.props.data.map(function (item, index) {
            return React.createElement(ReplyComp, { data: item, liked: Utils.getLiked(), end: Boolean(end === index) });
        });

        if (list.length > 5) {
            list.splice(4, 0, React.createElement(ContentADComp, null));
        } else {
            list.push(React.createElement(ContentADComp, null));
        }
...
var ReplyComp = React.createClass({
    ...
    render: function () {
        ...
        var boxStyle = { 'border-top': 'solid 20px #f0f0f0' };
        if (this.props.end) boxStyle = { 'border-top': 'solid 20px #f0f0f0', 'padding-bottom': '100px' };
        ...
        return React.createElement(
            ...
            React.createElement(
                ...
                React.createElement('img', { style: StylesReply.avatar, src: avatarUrl }),
                React.createElement(
                    'h1',
                    { style: [StylesReply.name, StylesReply.left1] },
                    decodeURI(data.nickname)
                ),
                React.createElement(
                    'span',
                    {
                        style: [StylesReply.time, StylesReply.left1, StylesReply.top1, StylesReply.font1] },
                    timeTxt
                ),
                React.createElement(
                    'span',
                    { style: [StylesReply.ordinal, StylesReply.top1, StylesReply.font1] },
                    data.seq,
                    '楼'
                )
            ),
            ...
            React.createElement(
                'div',
                { style: StylesReply.row2 },
                React.createElement(
                    'button',
                    { disabled: Boolean(this.state.liked), style: StylesReply.like1, onClick: this.likeHandler },
                    React.createElement('img', { src: imgPath + 'praise' + Number(this.state.liked) + '.png' })
                ),
                React.createElement(
                    'span',
    ...
    likeHandler: function (e) {
        ...
        $.ajax({
            ...
            data: {
                ...
                user_id: DataConfig.bfuid,
                nickname: DataConfig.nickname
            },
...
var RefreshLoadInfoComp = React.createClass({
    displayName: 'RefreshLoadInfoComp',
    ...
    render: function () {
        ...
        if (this.props.txtb1) {
            btn1Comp = React.createElement(
                'button',
                { style: [StylesAlert.btn0, StylesAlert[this.props.styleb1]],
        ...
        if (this.props.txtb2) {
            btn2Comp = React.createElement(
                'button',
                { style: [StylesAlert.btn0, StylesAlert[this.props.styleb2]],
        ...
        if (this.props.txtc) {
            contentTxtComp = React.createElement(
                'div',
                { style: [StylesReplyAll.centering, StylesAlert.font1] },
                this.props.txtc
            );
        }
        return React.createElement(
            'div',
            { style: StylesAlert.box },
            React.createElement('div', { style: StylesAlert.overlay, onClick: this.props.closeHandler }),
            React.createElement(
                'div',
                { ref: 'contentBox', style: StylesAlert[this.props.stylec] },
                contentTxtComp,
                btn1Comp,
                btn2Comp
            )
...
var WriteReplyComp = React.createClass({
    ...
    render: function () {
        ...
        var imgComp = React.createElement(
            'div',
            { style: StylesWriteReply.imgBox },
            React.createElement('img', { style: StylesWriteReply.img1, src: imgPath + 'upload0.png' }),
            React.createElement(
                'span',
                { style: StylesWriteReply.font1 },
                '添加图片'
            ),
            React.createElement('input', { style: StylesWriteReply.uploadInput, type: 'file', capture: 'camera', accept: 'image/*', id: 'file',
                onChange: this.imgChangeHandler, ref: 'uploadimg' })
        );
        //如果有图片数据,则显示预览和删除图片按钮
        if (this.state.imgData) {
            imgComp = React.createElement(
                'button',
                { style: StylesWriteReply.imgBox, onClick: this.delImgHandler },
                React.createElement('img', { style: StylesWriteReply.img1, src: this.state.imgData.base64 }),
                React.createElement(
                    'span',
                    { style: StylesWriteReply.font1 },
                    '删除图片'
        ...
        if (this.state.alertSendStatus !== '') {
            if (this.state.alertSendStatus !== AlertSendTxt.alertSend0) {
                alertComp = React.createElement(AlertComp, { animate: 'scale', txtc: this.state.alertSendStatus, stylec: 'content1', styleb1: 'btn1',
                    txtb1: '关闭',
                    closeHandler: this.closeAlertHandler });
            } else {
                alertComp = React.createElement(AlertComp, { animate: 'scale', txtc: this.state.alertSendStatus, stylec: 'content1' });
            }
        } else if (this.state.alert === 1) {
            alertComp = React.createElement(AlertComp, { animate: 'scale', txtc: '请输入内容或添加图片', stylec: 'content1', styleb1: 'btn1', txtb1: '关闭',
                closeHandler: this.closeAlertHandler });
        } else if (this.state.alert === 2) {
            alertComp = React.createElement(AlertComp, { animate: 'scale', txtc: '确定退出回复话题?', stylec: 'content1', styleb1: 'btn2', txtb1: '取消',
    ...
    imgChangeHandler: function (e) {
        ...
        lrz(this.refs.uploadimg.files[0], { width: 1024 }).then(function (rst) {
    ...
    sendImgData: function () {
        ...
        formData.append('image', this.state.imgData.file);
        formData.append('user', DataConfig.base64TokenUserId);
        xhr.send(formData);
...
var App = React.createClass({
    ...
    render: function () {
        ...
        var writeBTN = null;
        var infoComp = null;
        var refreshInfoComp = React.createElement(RefreshLoadInfoComp, { c1: 'pullDown', c2: 'pullDownLabel', t1: '下拉刷新...' });
        var loadMoreInfoComp = React.createElement(RefreshLoadInfoComp, { c1: 'pullUp', c2: 'pullUpLabel', t1: '上拉加载更多...' });
        if (this.state.initDataReady) {
            ...
            if (!(this.state.repliesList && this.state.repliesList.length)) {
                appADcomp = React.createElement(ContentADComp, null);
            }

            writeBTN = React.createElement(
                'div',
                { style: StylesReplyAll.writeBTNBox },
                React.createElement(
                    'button',
                    { style: StylesReplyAll.writeBTN, onClick: this.writeHandler },
                    React.createElement('img', { src: imgPath + 'post0.png' })
                )
            );
        }
        ...
        if (this.state.replyTop) {
            ...
            replyTopComp = React.createElement(ReplyComp, { data: this.state.replyTop, liked: Utils.getLiked() });
            repliesListTitleComp = React.createElement(
                'h1',
                { style: [StylesReplyAll.title, StylesReplyAll.two] },
                '大家的回复'
            );
        }
        ...
        if (this.state.repliesList) {
            replyListComp = React.createElement(ReplyListComp, { data: this.state.repliesList });
        }
        ...
        if (this.state.writeReplyVisible) {
            writeReplyComp = React.createElement(WriteReplyComp, { closeHandler: this.closeWriteReplyHandler });
        ...
        return React.createElement(
            ...
            React.createElement(
                'div',
                { ref: 'iscroll', style: page1Visible },
                React.createElement(
                    'div',
                    { style: { position: 'relative', 'min-height': document.documentElement.clientHeight } },
                    refreshInfoComp,
                    React.createElement(
                        'h1',
                ...
                writeBTN,
                React.createElement(DownloadADComp, null)
            ),
            writeReplyComp,
            StyleRulesGlobal
        );
    ...
    repliesListMoreGet: function (id, refresher) {
        if (id) {
            var key = null;
            if (this.state.repliesList.length) {
                key = this.state.repliesList[this.state.repliesList.length - 1].key;
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data && data.data && data.data.list && Array.isArray(data.data.list.posts) && data.data.list.posts.length) {
                        ...
                        if (!list) list = [];
                        var newList = list.concat(repliesListArray);
                        this.setState({ repliesList: newList });
                    }
                    if (refresher) refresher.refresh();
                }.bind(this),
                error: function (xhr, type) {
                    console.log(apiTopicList);
                    if (refresher) refresher.refresh();
                }.bind(this)
            });
        } else if (refresher) {
        ...
        if (id) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    DataConfig.userToken = Utils.b64btoa(data.token);
                    this.userLogin();
                }.bind(this),
                error: function (xhr, type) {
    ...
    userLogin: function () {
        $.ajax({
            ...
            data: {
                id: DataConfig.bfuid,
                nickname: Utils.b64btoa(DataConfig.nickname),
                token: DataConfig.userToken
            },
            error: function (xhr, type) {
 ...
 * 全局CSS
 */
var StyleRulesGlobal = React.createElement(Style, { rules: {
        '*, *:before, *:after': {
