                        ...
                        <div class="box">
                            <div class="box-body">
                                <div class="col-md-12 text-right">
                                    <a class="btn btn-success btn-flat" role="button" href="<?=base_url('/box/broadcast/add')?>"><i class="fa fa-plus"></i> 添加内容</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-12 user-list">
                        <div class="box">
                            <div class="box-header with-border">
                                <h3 class="box-title">列表<span class="text-red"></span></h3>
                            ...
                            <div class="box-body">
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        <?php $types = array('news'=>'新闻', 'video'=>'视频', 'collection'=>'合集', 'ad'=>'广告', 'matchteam'=>'比赛队伍', 'matchplayer'=>'比赛队员', 'matchvarious'=>'比赛方', 'html'=>'URL');?>
                                        <?php 
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <?php if ('html' == $item['type']) { echo $item['data']; } else { echo $item['ref_id']; }?>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                ...
                                                <a class="btn btn-flat btn-warning btn-xs" role="button" href="<?=base_url('/box/broadcast/add?id='.$item['id']).'&redirect='.current_url();?>"><i class="fa fa-edit"></i> 编辑</a>
...
$('#batch-remove,.btn-remove').on('click',function(){
    ...
    swal(alertConfig,function(){
        $.post('<?=base_url("/box/broadcast/remove")?>',{id : dataid},function(d){
            if(d == 'success'){
...
$('.user-list').on('click', '.editable-submit', function(){
	...
	$.post("<?=base_url('/box/broadcast/updatesort') ?>", { 'id':id, 'sort':input_val }, function(json){
	    if (!json.status) {
...
$("input[name='visible']").bootstrapSwitch({
    ...
    onSwitchChange: function (event, state) {
        $.ajax({
            method: "POST",
            url: "<?=base_url('/box/broadcast/upstatus') ?>",
